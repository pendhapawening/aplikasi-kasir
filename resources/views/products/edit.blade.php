            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category" id="category" required
                        ...
                        <option value="Non Coffee" {{ old('category', $product->category) == 'Non Coffee' ? 'selected' : '' }}>Non Coffee</option>
                ...
                <div class="md:col-span-2">
                    ...
                    @if($product->photo)
                        <div class="mb-2">
                            <img src="{{ asset('storage/' . $product->photo) }}" alt="{{ $product->name }}" class="w-32 h-32 object-cover rounded-lg">
                        </div>
                    @endif
                    <input type="file" name="photo" id="photo" accept="image/*"
                           class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-amber-500 focus:border-amber-500">
                    <p class="mt-1 text-sm text-gray-600">Biarkan kosong jika tidak ingin mengubah foto</p>
                    @error('photo')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
